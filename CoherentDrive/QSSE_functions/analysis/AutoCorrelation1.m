...
occ1=trace(operator1*operator2*rho);

OC12=tensor_contraction(C1,operator1*operator2,3,2);
Correlation(1)=tensor_contraction(OC12,conj(C1),[1,2,3],[1,2,3]);%/occ1;
...
for d=1:length(sites)
    ...
    occ2=trace(operator1*operator2*rho);
    ...
    Correlation(d+1)=tensor_contraction(OT1,OT2,[1,2],[1,2]);%/sqrt(occ1*occ2);
